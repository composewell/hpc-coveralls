...
import           Trace.Hpc.Coveralls.Config
    ( Config(Config, cabalFile, serviceName)
    , maybeToTestSuites
    )
...
getConfig hca = Config
    ...
    (optServiceName hca)
    (optRepoToken hca)
    (maybeToTestSuites $ argTestSuites hca)
...
main = do
    ...
    gitInfo <- getGitInfo
    coverallsJson <-
        generateCoverallsFromTix sn jobId gitInfo config mPkgNameVer
    ...
    unless (optDontSend hca) $ do
        ...
        case response of
            ...
            PostFailure msg -> do
                ...
                exitFailure
                where gitterUrl =
                          "https://gitter.im/guillaume-nargeot/hpc-coveralls"
